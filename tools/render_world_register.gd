...
func _run() -> void:
	...
	screen.focus_world_register_region("Landavol")
	await _save(viewport, "world_register_globe_landavol.png")

	screen.set_world_register_rotation_degrees(Vector3(0, 180, 0))
	await _save(viewport, "world_register_globe_opposite.png")

	screen.open_world_register("Pāwa Hitō")
	screen.focus_world_register_region("Pāwa Hitō")
	await _save(viewport, "world_register_globe_pawa.png")

	screen.set_world_register_mode(&"seams")
	screen.set_world_register_rotation_degrees(Vector3(-18, 34, 8))
	await _save(viewport, "world_register_globe_seams.png")
