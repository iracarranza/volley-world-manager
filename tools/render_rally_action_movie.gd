...
const ACTIONS := {
	"serve_standing": {"event": RallyEventModel.EventType.SERVE, "context": {"serve_style": "Standing", "action_power": 0.72}},
	"serve_jump_topspin": {"event": RallyEventModel.EventType.SERVE, "context": {"serve_style": "Jump Topspin", "action_power": 0.84}},
	"serve_jump_float": {"event": RallyEventModel.EventType.SERVE, "context": {"serve_style": "Jump Float", "action_power": 0.68}},
	"serve_hybrid": {"event": RallyEventModel.EventType.SERVE, "context": {"serve_style": "Hybrid", "action_power": 0.76}},
	"serve_sky_ball": {"event": RallyEventModel.EventType.SERVE, "context": {"serve_style": "Sky Ball", "action_power": 0.62}},
	"receive_dive": {"event": RallyEventModel.EventType.RECEPTION, "posture": "reaching", "recovery": "fall"},
	"receive_moving": {"event": RallyEventModel.EventType.RECEPTION, "posture": "moving", "ground_speed_mps": 3.1, "stride_cycle": 0.24},
	"receive_strained": {"event": RallyEventModel.EventType.RECEPTION, "posture": "off-axis", "ground_speed_mps": 2.4, "stride_cycle": 0.74},
	"set_front": {"event": RallyEventModel.EventType.SET, "context": {"set_posture": "standing", "back_set": false}},
	"set_back": {"event": RallyEventModel.EventType.SET, "context": {"set_posture": "standing", "back_set": true}},
	"set_moving": {"event": RallyEventModel.EventType.SET, "context": {"set_posture": "standing", "back_set": false}, "ground_speed_mps": 3.0, "stride_cycle": 0.24},
	"attack_power": {"event": RallyEventModel.EventType.ATTACK, "context": {"attack_type": "Power swing", "action_power": 0.82}},
	"attack_roll": {"event": RallyEventModel.EventType.ATTACK, "context": {"attack_type": "Roll shot", "action_power": 0.62}},
	"attack_dink": {"event": RallyEventModel.EventType.ATTACK, "context": {"attack_type": "Dink", "action_power": 0.38}},
	"attack_reaching": {"event": RallyEventModel.EventType.ATTACK, "context": {"attack_type": "Power swing", "action_power": 0.72, "attack_adjustment": "reaching"}},
	"attack_mistimed": {"event": RallyEventModel.EventType.ATTACK, "context": {"attack_type": "Power swing", "action_power": 0.72, "attack_adjustment": "mistimed"}},
	"attack_missed": {"event": RallyEventModel.EventType.ATTACK, "context": {"attack_type": "Power swing", "action_power": 0.72, "attack_adjustment": "missed"}},
	"block": {"event": RallyEventModel.EventType.BLOCK, "context": {}},
	"block_impact": {"event": RallyEventModel.EventType.BLOCK, "context": {"block_response": "impact"}},
	"block_tool": {"event": RallyEventModel.EventType.BLOCK, "context": {"block_response": "tool"}},
	"block_beaten": {"event": RallyEventModel.EventType.BLOCK, "context": {"block_response": "beaten"}},
...
func _build_stage(action_name: String, closeup: bool) -> PlayerActor3D:
	...
	actor.configure(900, true, action_name.replace("_", " ").to_upper(), "Right", {"height_cm": 191.0, "wingspan_cm": 197.0, "mass_kg": 84.0, "body_type": "Cani", "expression": "neutral", "appearance": {"palette_index": 0, "marking": "none"}})
...
func _record_action(actor: PlayerActor3D, spec: Dictionary) -> void:
	...
	actor.ground_speed_mps = float(spec.get("ground_speed_mps", 0.0))
	actor.stride_cycle = float(spec.get("stride_cycle", 0.0))
	...
	for frame in range(frames):
		...
		if seconds > HOLD_SECONDS + ACTION_SECONDS:
			...
			await process_frame
			continue
		var action_progress := clampf((seconds - HOLD_SECONDS) / ACTION_SECONDS, 0.0, 1.0)
		...
		actor.set_pose(event_type, elevation, phase, Vector2.RIGHT, true, Dictionary(spec.get("context", {})))
		await process_frame
...
func _record_blink(actor: PlayerActor3D) -> void:
	...
	var blink_duration := IdleBiomechanics.BLINK_CLOSE_SECONDS + IdleBiomechanics.BLINK_HOLD_SECONDS + IdleBiomechanics.BLINK_OPEN_SECONDS
