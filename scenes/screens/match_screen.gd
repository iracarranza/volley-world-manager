...
func _platform_surface(event: RallyEvent) -> Dictionary:
	...
	var outgoing: Dictionary = event.metadata.get("outgoing_trajectory", {})
	## Keep the empty branch typed. A ternary widens these two arrays to untyped
	## `Array` in Godot 4.7.1, then faults when assigning it to `Array[Resource]`
	## for presentation-only posture queries made before a result is loaded.
	var events: Array[Resource] = []
	if active_result != null:
		events = active_result.events
