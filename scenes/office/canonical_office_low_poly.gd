...
func _build_materials() -> void:
	_mat("floor", Color("7a6250"), 0.86)
	_mat("floor_alt", Color("8b715b"), 0.86)
	_mat("wall", Color("c8c0b1"), 0.95)
	_mat("wall_left", Color("9f9b93"), 0.95)
	_mat("trim", Color("6a5545"), 0.82)
	_mat("wood", Color("70513b"), 0.80)
	_mat("wood_light", Color("90684b"), 0.80)
	_mat("wood_dark", Color("49362c"), 0.84)
	_mat("fabric", Color("aaa49c"), 0.98)
	_mat("fabric_light", Color("d5d0c7"), 0.98)
	_mat("fabric_dark", Color("3b3d40"), 0.95)
	_mat("paper", Color("ded7c7"), 0.96)
	_mat("paper_alt", Color("b9ac8e"), 0.96)
	_mat("ink", Color("2c2c2b"), 0.90)
	_mat("metal", Color("4a4c4e"), 0.44, 0.12)
	_mat("glass", Color("6f939b"), 0.24)
	_mat("archive", Color("9b7b55"), 0.92)
	_mat("frame", Color("4b3a30"), 0.80)
	_mat("photo_a", Color("8c7c69"), 0.92)
	_mat("photo_b", Color("72817d"), 0.92)
	_mat("accent", Color("b48a38"), 0.64)
	_mat("red", Color("8e4d43"), 0.86)
	_mat("blue", Color("4e6670"), 0.86)
...
func _build_architecture() -> void:
	...
	# Broad plank rhythm, intentionally low-poly rather than textured realism.
	for i in 12:
	...
	# Baseboards keep the room readable in close cameras.
	...
	# Door and simple low-poly frame on the right wall.
	_box(root, "Door", Vector3(2.34, 1.02, 1.02), Vector3(0.10, 1.98, 0.82), "wood", Vector3(0, 0, 0))
...
func _build_window() -> void:
	...
	var z := -1.944
	# Window stays on the inner/left side of the desk wall.
	...
	# A few broad blind slats are enough to imply the object without texture work.
	for i in 5:
...
func _build_desk_zone() -> void:
	...
	# Desk frame: top + legs + right drawer pedestal.
	_box(root, "DeskTop", Vector3(1.05, 0.77, -1.47), Vector3(1.78, 0.09, 0.76), "wood_light")
	for x in [0.23, 1.87]:
		for z in [-1.74, -1.19]:
			_box(root, "DeskLeg", Vector3(x, 0.39, z), Vector3(0.08, 0.72, 0.08), "wood_dark")
	...
	# Archive remains physically under the desk.
...
func _build_manager_chair(parent: Node3D) -> void:
	...
	_cyl(c, "Post", Vector3(1.05, 0.27, -0.72), 0.055, 0.34, "metal", Vector3.ZERO, 8)
	# Five-spoke base, simplified to three visible bars in low-poly form.
	for angle in [-55.0, 0.0, 55.0]:
		_box(c, "ChairSpoke", Vector3(1.05, 0.11, -0.72), Vector3(0.08, 0.05, 0.58), "metal", Vector3(0, angle, 0))
	for p in [Vector3(0.80, 0.08, -0.52), Vector3(1.30, 0.08, -0.52), Vector3(1.05, 0.08, -1.00)]:
		_cyl(c, "Wheel", p, 0.045, 0.04, "metal", Vector3(90, 0, 0), 8)

func _build_desk_objects(parent: Node3D) -> void:
	var o := Node3D.new(); o.name = "DeskObjects"; parent.add_child(o)
	# Primary workspace objects retain distinct silhouettes and color values.
	...
	# Phone + answering machine form one communication cluster.
	...
	# Scouting / housing / kitchen references remain separate physical documents.
	...
	# Encyclopedia/reference books stand at the far-right back edge.
	for i in 4:
	...
	# Desk lamp, mug, and pencil cup are supporting props.
	...
	for i in 3:
		_cyl(o, "Pencil%02d" % i, Vector3(0.42 + float(i) * 0.02, 1.04 + float(i) * 0.015, -1.72), 0.008, 0.22, "accent", Vector3(0, 0, -4 + i * 5), 6)

func _build_sleep_zone() -> void:
	var root := Node3D.new(); root.name = "SleepZone"; add_child(root)
	# Bed is left side, same-room living rather than abstract quarters.
	...
	# A restrained rug separates the sleeping half without adding another system.
	...
	# Small bedside table keeps sleep-area props visually grounded.
	_box(root, "BedsideTop", Vector3(-0.68, 0.53, -0.77), Vector3(0.38, 0.06, 0.38), "wood_light")
	for x in [-0.82, -0.54]:
		for z in [-0.91, -0.63]:
			_box(root, "BedsideLeg", Vector3(x, 0.27, z), Vector3(0.05, 0.50, 0.05), "wood_dark")
	_cyl(root, "BedLampBase", Vector3(-0.68, 0.58, -0.77), 0.08, 0.025, "accent", Vector3.ZERO, 10)
	_cyl(root, "BedLampStem", Vector3(-0.68, 0.72, -0.77), 0.018, 0.27, "metal", Vector3.ZERO, 8)
...
func _build_guest_chair() -> void:
	...
	# Rest position is close to the desk; interviews only pull it a short distance.
	...
	for x in [-0.03, 0.39]:
		for z in [-0.74, -0.42]:
			_box(root, "GuestLeg", Vector3(x, 0.22, z), Vector3(0.055, 0.42, 0.055), "wood_dark", Vector3(0, -8, 0))
...
func _build_wall_state() -> void:
	...
	var z := -1.943
	# Calendar remains wall-mounted to the right of the desk/window.
	...
	# Automatic club-history frames begin sparse but have a stable anchor rail.
	for i in 4:
...
func _build_cameras() -> void:
	...
	# Same contract as the latest greybox packet.
