...
func _build_architecture() -> void:
	...
	# Current office continuity: window occupies the inner/left wall position;
	# calendar is the wall object to desk-right.
	_box(root, "Window", Vector3(0.35, 1.65, -ROOM_D/2.0 + 0.045), Vector3(1.0, 0.85, 0.06), "window")
...
func _build_furniture() -> void:
	...
	# Archive is physically stored beneath the desk, not a separate room corner.
	var storage := Node3D.new(); storage.name = "StorageZone"; add_child(storage)
	_box(storage, "Archive", Vector3(1.62, 0.23, -1.48), Vector3(0.48, 0.44, 0.50), "archive")

	var seating := Node3D.new(); seating.name = "FlexibleSeating"; add_child(seating)
	# Guest chair belongs within arm's-reach of the desk. It can be pulled only a
	# short distance into the open side of the desk for an informal interview.
	_box(seating, "GuestChairRest", Vector3(0.18, 0.44, -0.58), Vector3(0.50, 0.88, 0.50), "dark")
	var interview := Marker3D.new(); interview.name = "GuestChairInterviewAnchor"; interview.position = Vector3(0.28, 0, -0.38); seating.add_child(interview)

func _build_wall_state() -> void:
	var wall := Node3D.new(); wall.name = "WallZone"; add_child(wall)
	_box(wall, "Calendar", Vector3(1.62, 1.62, -1.945), Vector3(0.48, 0.64, 0.055), "paper")
	var frames := Node3D.new(); frames.name = "HistoryFrameAnchors"; wall.add_child(frames)
	for i in 4:
		var x := -0.75 + float(i) * 0.40
		_box(frames, "HistoryAnchor%02d" % (i + 1), Vector3(x, 2.12, -1.945), Vector3(0.28, 0.26, 0.055), "history")
...
func _build_cameras() -> void:
	...
	_camera(cams, "Interview", Vector3(1.55, 1.48, 0.42), Vector3(0.25, 1.0, -0.42), 48.0)
