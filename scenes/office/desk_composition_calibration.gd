...
func _apply() -> void:
	...
	# Lamp owns a clear patch at the back centre-left; no paper/book intersects it.
	...
	# All functional props are constrained inside the 1.78 x 0.76 m desktop
	# (x ~0.16..1.94, z ~-1.85..-1.09), with clearance from its physical edges.
	_place(office, "Journal", Vector3(1.40, 0.84, -1.53), Vector3(0.53, 0.050, 0.31), -3.0)
	_place(office, "JournalPageEdge", Vector3(1.40, 0.867, -1.53), Vector3(0.50, 0.011, 0.282), -3.0)
	_place(office, "TrainingClipboard", Vector3(0.67, 0.855, -1.48), Vector3(0.38, 0.030, 0.34), 7.0)
	_place(office, "ClipboardClip", Vector3(0.64, 0.88, -1.61), Vector3(0.11, 0.030, 0.050), 7.0)
	_place(office, "ScoutingBoard", Vector3(0.37, 0.84, -1.29), Vector3(0.38, 0.030, 0.27), 3.0)
	...
	_place(office, "AnsweringMachine", Vector3(1.70, 0.87, -1.32), Vector3(0.30, 0.12, 0.17), 4.0)
	var machine_light := office.find_child("MachineLight", true, false) as Node3D
	if machine_light != null: machine_light.position = Vector3(1.59, 0.93, -1.26)

	_place(office, "HousingFolder", Vector3(0.82, 0.84, -1.19), Vector3(0.33, 0.030, 0.20), -5.0)
	_place(office, "MealPad", Vector3(1.27, 0.84, -1.18), Vector3(0.29, 0.025, 0.19), 8.0)

	# Mug gets its own clear patch between Journal and communications.
	var mug := office.find_child("Mug", true, false) as Node3D
	if mug != null:
		mug.position = Vector3(1.53, 0.92, -1.34)
		mug.scale = Vector3(1.0, 1.0, 1.0)

	# Books stay along the back-left strip and do not enter the lamp footprint.
	var book_positions := [Vector3(0.34, 0.84, -1.70), Vector3(0.55, 0.84, -1.71), Vector3(0.58, 0.882, -1.70), Vector3(0.62, 0.920, -1.69)]
