class_name RegionalVenue3D
extends Node

## Build the eight major venues as *rooms*, not as floors.
##
...
##
## This is the production authority for venue appearance. MatchCourt3D applies
## it to real rally playback; the venue gallery subclasses it and photographs
## the same construction rather than keeping a second render-only copy.
...
var _roof_h := 14.0
var capture_gallery: bool = false


func _ready() -> void:
	if not capture_gallery:
		return
...
## Stable persisted identity for a regional venue. The region name is the save
## contract; the lowercase id remains an implementation detail of the builder.
static func venue_id_for_region(region_name: String) -> String:
	var folded := region_name.to_lower()
	if "pāwa" in folded or "pawa" in folded:
		return "Pāwa Hitō"
	if "spëddigh" in folded or "speddigh" in folded:
		return "Spëddigh"
	if "blôc" in folded or "bloc" in folded:
		return "Blôc du Larg"
	if "xérvu" in folded or "xervu" in folded:
		return "Xérvu"
	if "taktikã" in folded or "taktika" in folded:
		return "Taktikã"
	if "a'ace" in folded or "aace" in folded:
		return "A'ace"
	if "ĭspayk" in folded or "ispayk" in folded:
		return "Ĭspayk"
	## Minor regions still own their persisted venue identity even though the
	## current art set has only the eight major rooms. Their renderer falls back
	## below without rewriting save data or the scoreboard to Landavol.
	return VolleyballRegions.canonical_name(region_name)


static func selectable_venues() -> Array[String]:
	return [
		"Landavol", "Spëddigh", "Pāwa Hitō", "Blôc du Larg",
		"Xérvu", "Taktikã", "A'ace", "Ĭspayk",
	]


func configure_existing(court: Node3D, venue_identity: String) -> void:
	if court == null:
		return
	_court = court
	...
	var identity := venue_id_for_region(venue_identity)
	var venue := _venue_for_identity(identity)
	_open_air = bool(venue.get("open_air", false))
	_tight = bool(venue.get("tight", false))
	_arena()
	_roof_lights(str(venue.get("id", "landavol")))
	_fixtures(str(venue.get("id", "landavol")))
	_floor_for(str(venue.get("id", "landavol")))
	var build: Callable = venue.get("build", func(): pass)
	build.call()
	_frame_broadcast()


func _venue_for_identity(identity: String) -> Dictionary:
	for raw_venue in _venues():
		var venue := Dictionary(raw_venue)
		if str(VENUE_REGION.get(str(venue.get("id", "")), "")) == identity:
			return venue
	return Dictionary(_venues()[0])
