...
## NOTE `contact_control` is continuous and must not be turned into a cliff -- RALLY_SIMULATOR_NOTES.md
...
## NOTE How much of the time between the pass and the set's release a blocker can actually use to move -- RALLY_SIMULATOR_NOTES.md
const ASSIST_COMMIT_FLIGHT_SECONDS: float = 0.65

## NOTE What a wall that *committed* keeps when the ball comes quick -- RALLY_SIMULATOR_NOTES.md
...
## NOTE How decisively the block has to beat the swing for each outcome -- RALLY_SIMULATOR_NOTES.md
...
## NOTE What this blocker means to do with their hands -- RALLY_SIMULATOR_NOTES.md
func _block_hands_intent(
	...
	## NOTE Late and low, not behind on the contest -- RALLY_SIMULATOR_NOTES.md
...
## NOTE What the hands are *trying to do*, which is a different axis from where the wall stands -- RALLY_SIMULATOR_NOTES.md
...
## NOTE where an own-side delivery arrives, in sigmas from the aim -- RALLY_SIMULATOR_NOTES.md
...
## NOTE How far a delivery is allowed to stray before it stops being a delivery -- RALLY_SIMULATOR_NOTES.md
...
## NOTE Where a pass stops supporting a quicker call and starts forcing a slower one -- RALLY_SIMULATOR_NOTES.md
...
## NOTE The three tempo gates, re-sited on the distributions they actually cut -- RALLY_SIMULATOR_NOTES.md
...
## NOTE How far from neutral a blended commitment has to sit to pull the tempo every time it can -- RALLY_SIMULATOR_NOTES.md
...
## NOTE How much of the pre-set window the hitter is credited with -- RALLY_SIMULATOR_NOTES.md
...
## NOTE How much the attacker is favoured when swing and dig are equally good -- RALLY_SIMULATOR_NOTES.md
const DIG_ATTACKER_ADVANTAGE: float = -0.04

## NOTE One defender is not a whole defence -- RALLY_SIMULATOR_NOTES.md
...
## NOTE What a metre of dragged-back contact costs the swing -- RALLY_SIMULATOR_NOTES.md
...
## NOTE FLAGGED -- RALLY_SIMULATOR_NOTES.md
...
## NOTE What each player *did*, in condition, this rally -- RALLY_SIMULATOR_NOTES.md
...
## NOTE Names available to player-facing narration, filled in as the rally reaches each contact -- RALLY_SIMULATOR_NOTES.md
...
func resolve(
	...
	## NOTE weights are normalised by their own total, so this is a genuine 0-1 -- RALLY_SIMULATOR_NOTES.md
	...
	## NOTE Judged from the bodies, not the formation -- RALLY_SIMULATOR_NOTES.md
	...
	## NOTE Where this ball is passed, which is not where it lands -- RALLY_SIMULATOR_NOTES.md
	...
	var set_arc := _set_arc(
		setter, assignment.tempo, float(result.set_quality),
		## NOTE Where the ball was, not how high the setter can reach -- RALLY_SIMULATOR_NOTES.md
		delivered_set_contact_height \
	...
	## NOTE The instant the ball leaves the setter's hands -- RALLY_SIMULATOR_NOTES.md
...
func _resolve_home_serve(
	...
	## NOTE `opponent_setter_release` is resolved above, before the pass, because the pass is thrown at it -- RALLY_SIMULATOR_NOTES.md
	var opponent_lineup: RotationLineup = opponent_team.current_lineup()
...
func _resolve_opponent_transition(
	...
	## NOTE how long these volis have been running when the pass is played -- RALLY_SIMULATOR_NOTES.md
	...
	if opponent_setter == null:
		opponent_setter = opponent_team.setter() as VolleyballPlayer
	## NOTE Consume the selection rather than rebuilding a second one -- RALLY_SIMULATOR_NOTES.md
	...
	## NOTE A release is a run, so this leg is `transition` -- RALLY_SIMULATOR_NOTES.md
	...
	## NOTE Per lane, not the general floor -- RALLY_SIMULATOR_NOTES.md
	...
	## NOTE The authoritative value, once the real hitter and contact point are known -- RALLY_SIMULATOR_NOTES.md
	...
	## NOTE Decide roll-against-swing on the set that was delivered -- RALLY_SIMULATOR_NOTES.md
	...
	## NOTE When the setter actually touches the ball -- RALLY_SIMULATOR_NOTES.md
	...
	if opponent_set_event != null:
		...
		opponent_set_event.metadata["set_release_height_meters"] = \
			opponent_release_height
		## NOTE The ball this set was resolved against -- RALLY_SIMULATOR_NOTES.md
	...
	## NOTE The full shot, to where it is actually aimed -- RALLY_SIMULATOR_NOTES.md
	...
	## NOTE The swing the wall actually met, re-read after the truncation above -- RALLY_SIMULATOR_NOTES.md
	...
	## NOTE The dig's flight budget is the flight that was drawn -- RALLY_SIMULATOR_NOTES.md
...
func _resolve_home_continuation(
	...
	## NOTE The same read the opponent's setter makes, off the same base -- RALLY_SIMULATOR_NOTES.md
	...
	## NOTE What this setter can actually deliver off this ball -- RALLY_SIMULATOR_NOTES.md
	...
	## NOTE Where this swing is aimed -- RALLY_SIMULATOR_NOTES.md
	var attack_choice := _choose_attack_target(
	...
	if dug:
		cont_dig_pass = _dig_pass_result(
			...
			cont_dig_control,
			## NOTE the arrival was `{}`, so every continuation dig stretched by a constant 0.294 -- RALLY_SIMULATOR_NOTES.md
...
func _form_opponent_block(
	...
	## NOTE The assist cannot have crossed the court before the setter touched it -- RALLY_SIMULATOR_NOTES.md
...
## NOTE Where the ball met the tape, as the realised contact rather than a position assembled beside it -- RALLY_SIMULATOR_NOTES.md
...
## NOTE Whose hands, when the ball met any -- RALLY_SIMULATOR_NOTES.md
...
## NOTE Where a ball deflected off the wall comes down on the hitter's own side -- RALLY_SIMULATOR_NOTES.md
...
## NOTE where a missed attack lands: past the line, into the net, or short -- RALLY_SIMULATOR_NOTES.md
...
## NOTE Where a swing that misses actually lands -- RALLY_SIMULATOR_NOTES.md
func _approach_budget(
	...
	## NOTE The run-up's clock is bounded by the run-up, not by the set -- RALLY_SIMULATOR_NOTES.md
...
## NOTE Where this hitter aims, chosen continuously from the actual open floor -- RALLY_SIMULATOR_NOTES.md
...
## NOTE Who on the opponent plays this ball, through the same search the home side uses -- RALLY_SIMULATOR_NOTES.md
...
func _reachable_attack_contact(
	...
	## NOTE the journey was charged against the set's flight alone -- the double-charge `_approach_budget` fixes -- RALLY_SIMULATOR_NOTES.md
...
## NOTE Where the body stands, given where the contact is -- RALLY_SIMULATOR_NOTES.md
...
func _choose_opponent_attack(
	...
	## NOTE drawn ahead of the branch so draw count is branch-independent; the shot is provisional -- RALLY_SIMULATOR_NOTES.md
...
## NOTE `stage_server` is false when the caller wants a *resting* arrangement rather than the opening snapshot -- RALLY_SIMULATOR_NOTES.md
func _initial_home_positions(
	...
	for slot_number in range(1, 7):
		...
		if defensive_plan != null:
			if receiving:
				## NOTE `enabled` is checked here now, and was not -- RALLY_SIMULATOR_NOTES.md
...
func _physical_playback_profile(
	...
	return {
		...
		## NOTE How fast this body can actually be moved across the floor -- RALLY_SIMULATOR_NOTES.md
...
## NOTE Where a defending six stands while the ball is being attacked at them -- RALLY_SIMULATOR_NOTES.md
...
func _floor_phase_positions(
	...
	## NOTE The wall, at its two shoulders -- RALLY_SIMULATOR_NOTES.md
...
## NOTE What tempo the opponent calls on this ball -- RALLY_SIMULATOR_NOTES.md
func _tempo_call(
	setter: VolleyballPlayer,
	requested_tempo: int,
	pass_quality: float,
...
func _opponent_defensive_plan(opponent_team: Resource) -> Resource:
	...
	## NOTE Deliberately no floor preset on top -- RALLY_SIMULATOR_NOTES.md
	for zone_type in [
...
func _travel(
	...
	## NOTE The route no longer chooses the orientation -- RALLY_SIMULATOR_NOTES.md
...
## NOTE What the home wall is allowed to know before the opposing setter releases -- RALLY_SIMULATOR_NOTES.md
...
## NOTE primary on the read crossing, assistant's shoulder from where they close -- RALLY_SIMULATOR_NOTES.md
...
## NOTE A ball coming off the block, timed by geometry rather than a constant -- RALLY_SIMULATOR_NOTES.md
func _block_deflection_trajectory(
	...
	## NOTE What `BlockDeflectionModel` said this ball did -- RALLY_SIMULATOR_NOTES.md
	...
	## NOTE Where the ball actually met the hands, when the resolver proved it -- RALLY_SIMULATOR_NOTES.md
...
## NOTE The ball is contacted where the hitter can be, not where the set wanted them -- RALLY_SIMULATOR_NOTES.md
...
## NOTE The lateness that survives the clamp above -- RALLY_SIMULATOR_NOTES.md
...
## NOTE When the swing reached the tape, or `fallback` if this arc cannot say -- RALLY_SIMULATOR_NOTES.md
...
## NOTE The flight a swing actually produced -- RALLY_SIMULATOR_NOTES.md
...
func _swing_arc(
	...
	## NOTE The resolver's own angle, when the ball is going where the resolver sent it -- RALLY_SIMULATOR_NOTES.md
...
## NOTE The same swing, drawn only as far as the block -- RALLY_SIMULATOR_NOTES.md
func _truncated_arc(
...
func _ball_trajectory(
	...
	## NOTE Where the ball actually was, vertically, at each end of this flight -- RALLY_SIMULATOR_NOTES.md
	start_height: float = NAN,
	end_height: float = NAN,
	## NOTE Which launch this flight belongs to -- RALLY_SIMULATOR_NOTES.md
	flight_id: String = "",
) -> Dictionary:
...
## NOTE Put the launch state on the published flight, so nothing has to rebuild it -- RALLY_SIMULATOR_NOTES.md
...
## NOTE What a platform contact was *for*, published beside what it did -- RALLY_SIMULATOR_NOTES.md
...
## NOTE How high a pass goes above the platform that played it, from a shank to a perfect one -- RALLY_SIMULATOR_NOTES.md
...
## NOTE The ball that physically leaves a successful floor dig -- RALLY_SIMULATOR_NOTES.md
...
func _physical_platform_dig_result(
	...
	if digger == null:
		return {}
	## NOTE The ball's height, where the incoming flight can state it -- RALLY_SIMULATOR_NOTES.md
...
## NOTE The keep-alive ball a successful attack-coverage contact launches -- RALLY_SIMULATOR_NOTES.md
...
func _reception_pass_result(
	...
	## NOTE How high the ball was put up, and therefore how long the setter has -- RALLY_SIMULATOR_NOTES.md
	var pass_contact_height := GeometricAttackPromotionModel \
...
## NOTE Whether a defender stayed on their feet, and what happened to them if not -- RALLY_SIMULATOR_NOTES.md
...
## NOTE Where each pose begins, as a *shortfall* against the posture's own norm -- RALLY_SIMULATOR_NOTES.md
...
## NOTE How hard the ball has to arrive to drive an average voli off it -- RALLY_SIMULATOR_NOTES.md
...
## NOTE Where a contact stops being planted, per branch -- RALLY_SIMULATOR_NOTES.md
...
## NOTE How well this voli stays on their feet, as three separate questions -- RALLY_SIMULATOR_NOTES.md
...
## NOTE What a dig cost the defender, on the same four bands as a reception -- RALLY_SIMULATOR_NOTES.md
func _dig_recovery(
	defender: VolleyballPlayer,
...
## NOTE What a receiver brings to a serve reception, before the serve itself is weighed -- RALLY_SIMULATOR_NOTES.md
...
func _contact_recovery_state(
	...
	## NOTE Ordered, worst first, on one scale -- so a worse contact can never land in a gentler pose than a better one -- RALLY_SIMULATOR_NOTES.md
...
## NOTE Record what a contact cost the player who made it, and charge it -- RALLY_SIMULATOR_NOTES.md
...
## NOTE The seconds each player still owes, in the shape `choose_claimant` takes -- RALLY_SIMULATOR_NOTES.md
func _recovering_count(at_time: float) -> int:
	return _recovery_time_penalties(at_time).size()



## NOTE How long each blocker is still off the floor when the ball comes down -- RALLY_SIMULATOR_NOTES.md
...
## NOTE Which way each of these bodies is set, for the claim search -- RALLY_SIMULATOR_NOTES.md
...
## NOTE Who is physically taking this second contact, and from where -- RALLY_SIMULATOR_NOTES.md
...
## NOTE Puts the second-contact claim onto the event that resulted from it -- RALLY_SIMULATOR_NOTES.md
...
## NOTE How much this voli calls for a ball nobody has been assigned -- RALLY_SIMULATOR_NOTES.md
...
## NOTE What a teammate nearby is worth, which is not always positive -- RALLY_SIMULATOR_NOTES.md
...
func _resolve_overpass_attack(
	...
	## NOTE the attacker's live position, from their own side's map -- RALLY_SIMULATOR_NOTES.md
...
func _spatial_setter_choice(
	...
	target: Vector2,
	available_time: float,
	## NOTE How long these volis have already been running when the pass is played -- RALLY_SIMULATOR_NOTES.md
	head_start_seconds: float = 0.0,
	## NOTE Where the setter is running while the head start runs, which is not where the ball turns out to go -- RALLY_SIMULATOR_NOTES.md
	...
	for candidate in candidates:
		...
		## NOTE How confidently, not merely whether -- RALLY_SIMULATOR_NOTES.md
		...
		var reach_margin := float(arrival.get("reach_margin_meters", 0.0))
		## NOTE The active setter's responsibility replaces the plan's, it does not stack on it -- RALLY_SIMULATOR_NOTES.md
		var score := _second_contact_claim_score(
	...
	if claimants.size() >= 2:
		var gap := float(claimants[0].score) - float(claimants[1].score)
		best["claim_margin"] = gap
		## NOTE A seam is a failure to delegate, so cohesion is the width of it -- RALLY_SIMULATOR_NOTES.md
		best["seam_conflict"] = gap < _seam_margin(
			_player_by_id(candidates, int(claimants[0].id)),
			_player_by_id(candidates, int(claimants[1].id)),
		)
...
## NOTE How wide the window is in which two volis both think the ball is theirs -- RALLY_SIMULATOR_NOTES.md
...
## NOTE the designated setter, or the plan's cover if they took the first contact -- RALLY_SIMULATOR_NOTES.md
...
## NOTE When each event physically happened, on the clock the rally was simulated on -- RALLY_SIMULATOR_NOTES.md
...
## NOTE Where the ball was when each contact was made, carried forward once -- RALLY_SIMULATOR_NOTES.md
...
func _stamp_realised_contact_heights(result: Resource) -> void:
	...
	for raw_event in result.events:
		...
		## NOTE An action event is not a ball contact, and must not become one by standing between two -- RALLY_SIMULATOR_NOTES.md
		...
		if previous != null \
			...
			## NOTE Only when the incoming flight actually ends at this contact -- RALLY_SIMULATOR_NOTES.md
			...
			if source == "resolved":
				...
				## NOTE Same derivation the platform resolver now uses, so the height a contact is *resolved* at and the height it is *drawn* at are one function rather than two that agree by inspection -- RALLY_SIMULATOR_NOTES.md
				event.metadata["ball_contact_height_meters"] = maxf(
					realised_flight_end_height(incoming), 0.0
				)
				event.metadata["ball_contact_height_source"] = \
					"incoming_launch_integrated"
		## NOTE `outgoing.start == C` is the third term and it is not closed here -- RALLY_SIMULATOR_NOTES.md
		previous = event
...
func _finalize_rally_timeline(result: Resource) -> void:
	...
	for event_resource in result.events:
		var event: Resource = event_resource
		var metadata: Dictionary = event.metadata
		## NOTE What the resolver itself said, kept before this function replaces it -- RALLY_SIMULATOR_NOTES.md
		...
		event.metadata = metadata
		## NOTE The ball's own motion advances the rally clock -- RALLY_SIMULATOR_NOTES.md
	...
	for event_index in range(result.events.size()):
		...
		if flight_time <= 0.0:
			match int(event.event_type):
				...
				## NOTE A successful floor dig no longer arrives here -- RALLY_SIMULATOR_NOTES.md
				RallyEventModel.EventType.DIG, \
...
func _add_event(
	...
	## NOTE What every body on court still owes, on the contact that samples it -- RALLY_SIMULATOR_NOTES.md
	...
	## NOTE Where the body that made this contact was standing -- RALLY_SIMULATOR_NOTES.md
...
func _setter_option_terms(
	...
	## NOTE How well this setter knows this hitter -- RALLY_SIMULATOR_NOTES.md
	var trust := (
...
## NOTE How hard the ball itself is to handle, on top of how well it was struck -- RALLY_SIMULATOR_NOTES.md
...
func _fallback_hitter(
	...
	## NOTE The middle, on a pass that allows one -- RALLY_SIMULATOR_NOTES.md
	...
	if RallyFeatureFlagsModel.ENABLE_HOME_MIDDLE_OFFENSE:
		...
		for slot_number in range(1, 7):
			...
			if is_middle and not quick_is_on:
				continue
			## NOTE The back row swings too, on a ball that allows it -- RALLY_SIMULATOR_NOTES.md
			if not front_row:
			...
			## the set to read.
				...
				score += PIPE_OPTION_BONUS \
					- (1.0 - _rating(contender, "transition_speed")) \
						* PIPE_TRAVEL_COST
			## NOTE And spread the ball, because ability alone is still one lane -- RALLY_SIMULATOR_NOTES.md
...
## NOTE A blocker reads the arm, and a fast arm gives them less of it to read -- RALLY_SIMULATOR_NOTES.md
...
## NOTE What a defender can tell about a swing before it happens -- RALLY_SIMULATOR_NOTES.md
...
func _blocker_close_terms(
	...
	## NOTE Blocking closes through the shared locomotion model like every other movement in the engine -- RALLY_SIMULATOR_NOTES.md
	...
	return {
		"fraction": fraction,
		## NOTE Where this blocker actually ended up, in normalised court x -- RALLY_SIMULATOR_NOTES.md
...
## NOTE One swing, wherever in the rally it happens -- RALLY_SIMULATOR_NOTES.md
...
## NOTE Gate E -- RALLY_SIMULATOR_NOTES.md
...
## NOTE The serve -- RALLY_SIMULATOR_NOTES.md
func _canonical_serve(
	...
	## NOTE The serve as a flight that can be asked a question about a time -- RALLY_SIMULATOR_NOTES.md
...
## NOTE Gate E promotion -- RALLY_SIMULATOR_NOTES.md
...
## NOTE Did the wall *shape* this swing, though it never touched it? -- RALLY_SIMULATOR_NOTES.md
...
## NOTE How much a team's commitment moves the bar a swing has to clear -- RALLY_SIMULATOR_NOTES.md
...
## NOTE How often this serve misses, given how much the server is asking of it -- RALLY_SIMULATOR_NOTES.md
...
## NOTE One set, wherever in the rally and whichever side of the net -- RALLY_SIMULATOR_NOTES.md
func _set_execution(
	setter: VolleyballPlayer,
...
## NOTE How widely a player's execution scatters around what they are capable of -- RALLY_SIMULATOR_NOTES.md
...
func _execution_error(
	...
	## NOTE Normal, not uniform on [-spread, spread] -- RALLY_SIMULATOR_NOTES.md
...
## NOTE Where an own-side delivery lands, given where it was aimed and how well it was executed -- RALLY_SIMULATOR_NOTES.md
...
## NOTE how high the ball climbs before accuracy starts paying, and how fast -- RALLY_SIMULATOR_NOTES.md
...
func _delivered_point(
	...
	## NOTE How far the ball climbs above the release, not its absolute apex and not only the rescue portion -- RALLY_SIMULATOR_NOTES.md
...
## NOTE One dig, wherever in the rally it happens -- RALLY_SIMULATOR_NOTES.md
...
## NOTE The same dig, with its working shown -- RALLY_SIMULATOR_NOTES.md
...
func _read_error_meters(
	...
	var signature := BallContactSignature.create(
		&"flight",
		## NOTE The pace the ball actually left the contact at, when the record knows it -- RALLY_SIMULATOR_NOTES.md
	...
	## NOTE Measured from the flight's own clock, not the rally's -- RALLY_SIMULATOR_NOTES.md
...
## NOTE Whether this dig comes up, against the swing that was actually hit -- RALLY_SIMULATOR_NOTES.md
...
## NOTE How poor a set has to be before the hitter gives up the swing -- RALLY_SIMULATOR_NOTES.md
...
## NOTE The lane and tempo for a swing nobody called a play for -- RALLY_SIMULATOR_NOTES.md
func _fallback_assignment(
	...
	## NOTE The lane a hitter *can* be set, rather than the one their rotation slot implies -- RALLY_SIMULATOR_NOTES.md
...
## NOTE One of these hitters, in proportion to how good an option they are -- RALLY_SIMULATOR_NOTES.md
...
func _serve_landing_point(
	...
	## NOTE Where the zone actually is, given who is standing in the way -- RALLY_SIMULATOR_NOTES.md
...
## NOTE The best point to serve near a requested zone, given where the passers are *and* what this server can actually hit -- RALLY_SIMULATOR_NOTES.md
...
## NOTE Where every voli on the receiving side stands to take a serve, by player id -- RALLY_SIMULATOR_NOTES.md
...
## NOTE Where the four volis who are neither passing nor setting go while the pass is in the air -- RALLY_SIMULATOR_NOTES.md
...
## NOTE The same idea for the other side of the net, at the fidelity that side has -- RALLY_SIMULATOR_NOTES.md
...
## NOTE Where the attacking side goes while their own spike is in the air -- RALLY_SIMULATOR_NOTES.md
...
## NOTE The blocking side going after a ball that came off their own hands -- RALLY_SIMULATOR_NOTES.md
...
## NOTE Intended shot shape for a set, by tempo -- RALLY_SIMULATOR_NOTES.md
...
## NOTE How high a set goes above the hands that will hit it, by tempo -- RALLY_SIMULATOR_NOTES.md
const SET_CLEARANCE_BY_TEMPO: Array[float] = [0.15, 0.60, 1.30, 2.20]
## NOTE A fast set is struck before its apex, and the model could not say so -- RALLY_SIMULATOR_NOTES.md
...
## NOTE The flight of a set, timed by how high it was put up -- RALLY_SIMULATOR_NOTES.md
...
## NOTE What a set's pace comes from, as a multiple of the baseline -- RALLY_SIMULATOR_NOTES.md
...
func _set_arc(
	...
	return {
		...
		## NOTE Where this ball starts and where it arrives, in absolute metres -- RALLY_SIMULATOR_NOTES.md
		"release_height_meters": release_height_meters,
		"arrival_height_meters": hitter_contact_height_meters,
	}
