...
| **M4 — physical platform contact** | **IN PROGRESS / AUTHORED SHADOW + DIG ROLLOUT** | One shared authored T1--T3 relation now evaluates every reception/dig and all coverage contacts without event-family bands. Controlled dig owns a development-only outgoing launch; production remains closed because free-flight shanks require M5 interception rather than a forced setter endpoint. Coverage state is complete but its full keep-alive selection preference remains a genuine policy boundary. | [`PLATFORM_CONTACT.md`](PLATFORM_CONTACT.md), [`PLATFORM_AUTHORED_CALIBRATION.md`](../review/PLATFORM_AUTHORED_CALIBRATION.md), [`PLATFORM_PHYSICS_AUTHORITY_BOUNDARY.md`](../review/PLATFORM_PHYSICS_AUTHORITY_BOUNDARY.md) |
| **M5 — free-flight and interception authority** | **DEVELOPMENT-BUILT + CERTIFIED; PRODUCTION GATED** | Exit condition met *in development*: outgoing launch is authoritative and immutable, intended recipient ≠ endpoint/interceptor, realised segment is a prefix of the free flight, shanks are intercepted en route, and the last unresolved same-side exit — an **overpass**, the receiving side's ordinary first team contact — now resolves through `OverpassActionSystem` at both live `crossed_net_unresolved` exits (control and attack). Not promoted to production: `ENABLE_PHYSICAL_PLATFORM_DIG` stays `false`, so production still runs the legacy dig and the free-flight/overpass path fires 0× in 1,200 ordinary rallies. Promotion is the open decision — its stated blocker (an ungoverned overpass) is now cleared, but flipping the flag changes the outcome distribution and is a paired-census balance question, not a fit. | [`CONTACT_AND_BALL_FLIGHT.md`](CONTACT_AND_BALL_FLIGHT.md), [`OVERPASS_ACTION_HANDOFF.md`](../review/OVERPASS_ACTION_HANDOFF.md), [`FREE_FLIGHT_INTERCEPTION.md`](../review/FREE_FLIGHT_INTERCEPTION.md) |
