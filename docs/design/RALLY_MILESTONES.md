...
| **M4 — physical platform contact** | **IN PROGRESS / AUTHORED SHADOW + DIG & COVERAGE ROLLOUT** | One shared authored T1--T3 relation now evaluates every reception/dig and all coverage contacts without event-family bands. Controlled dig and attack coverage each own a development-gated physical outgoing launch through the same shared resolver; both route their free flight through M5 interception, so neither forces a setter endpoint. Coverage's keep-alive selection is now governed — its intended recipient is the actor the existing second-contact policy names, with the coverer excluded, adding no new ranking — and was the last platform family still fabricating its ball. Production remains closed (`ENABLE_PHYSICAL_PLATFORM_DIG=false`), byte-neutral until the flag opens; flipping it is a paired-census balance question, not a fit. | [`PLATFORM_CONTACT.md`](PLATFORM_CONTACT.md), [`PLATFORM_AUTHORED_CALIBRATION.md`](../review/PLATFORM_AUTHORED_CALIBRATION.md), [`COVERAGE_KEEP_ALIVE.md`](../review/COVERAGE_KEEP_ALIVE.md), [`PLATFORM_PHYSICS_AUTHORITY_BOUNDARY.md`](../review/PLATFORM_PHYSICS_AUTHORITY_BOUNDARY.md) |
