...
| **M11 — contact-state continuity (§5)** | **IN PROGRESS** | Every contact's incoming leg ends where its outgoing leg begins, in time *and* in height, on the record the drawing reads. **Not M6 restated.** Same-lineage says the ball was handed along; it does not say the ball's state is continuous across the handover, and a 2.79 m height discontinuity satisfies every gate M6 and M8 ran. Numbered after M10 by date of definition and not by dependency: it is M10's precondition, and five passes of it are already delivered — inside M8's row, which is why it is being given one of its own. Exit condition is a §5 census whose criterion is that the two legs *agree*, rather than that each derived a number. The old criterion was too weak and this is how that was found: the reception read authoritative while deriving its height from the serve under 9.8 m/s² for a ball falling at 21.009. | [`CONTACT_AND_BALL_FLIGHT.md`](CONTACT_AND_BALL_FLIGHT.md) §5; [`BLOCK_REALISED_CONTACT.md`](../review/BLOCK_REALISED_CONTACT.md), [`CONTACT_HEIGHT_CHAIN.md`](../review/CONTACT_HEIGHT_CHAIN.md), [`SERVE_RECEPTION_HEIGHT_SEAM.md`](../review/SERVE_RECEPTION_HEIGHT_SEAM.md) |
