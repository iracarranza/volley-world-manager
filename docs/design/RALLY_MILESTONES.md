...
| **M4 — physical platform contact** | **DIG + COVERAGE IN PRODUCTION; RECEPTION BUILT, PRODUCTION-GATED** | One shared authored T1--T3 relation evaluates every reception/dig and all coverage contacts. **Dig and coverage are promoted** (`ENABLE_PHYSICAL_PLATFORM_DIG=true`): each launches a physical outgoing ball through the shared resolver, routed through M5 so neither forces a setter endpoint; suite 2161/2161, rollouts PASS, balance healthy (swing 1.032, stuff 0.120, dig 0.353); a set fed by a physical interception reports the realised prefix so the one-ball chain holds by identity. **Reception is built and dev-certified** behind its own gate `ENABLE_PHYSICAL_RECEPTION` (default false) — the home first-ball path was retrofitted with the M5 branch it lacked, opponent routes through the transition, paired rollout 14/14. Reception production is **gated**: flipping it trips the deferred short-leg movement-agreement instrument limit plus first-ball delivered-height/setter-endpoint invariants, which must be resolved on their own terms, not worked around. **M4 is DONE only when reception also has production authority.** | [`PLATFORM_CONTACT.md`](PLATFORM_CONTACT.md), [`PLATFORM_DIG_PROMOTION.md`](../review/PLATFORM_DIG_PROMOTION.md), [`COVERAGE_KEEP_ALIVE.md`](../review/COVERAGE_KEEP_ALIVE.md), [`PLATFORM_AUTHORED_CALIBRATION.md`](../review/PLATFORM_AUTHORED_CALIBRATION.md) |
