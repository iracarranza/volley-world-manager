...
| **M4 — physical platform contact** | **IN PROGRESS / OWNER AUTHORITY BOUNDARY** | Reception, controlled dig, emergency dig and attack coverage produce an outgoing ball from incoming ball + body/contact state + intent/selection + execution, not from event-specific apex bands. Intent publication, shipped-model transfer characterization, controlled attribute leverage and parameter-free contact-context instrumentation are done. T1/T2 lack measured magnitudes; T3 has only an aggregate order-of-magnitude bound. The next code requires either the documented capture route or explicit authorization of shared game-calibration parameters. | [`PLATFORM_CONTACT.md`](PLATFORM_CONTACT.md), [`PLATFORM_INTENT.md`](../review/PLATFORM_INTENT.md), [`PLATFORM_TRANSFER.md`](../review/PLATFORM_TRANSFER.md), [`PLATFORM_ATTRIBUTE_LEVERAGE.md`](../review/PLATFORM_ATTRIBUTE_LEVERAGE.md), [`PLATFORM_PHYSICS_AUTHORITY_BOUNDARY.md`](../review/PLATFORM_PHYSICS_AUTHORITY_BOUNDARY.md) |
...
difficult contacts separate survival as well. The envelope half still needs
T1/T2 authored and is not started. A primary-evidence review found outgoing
...
skill. That is a genuine authoring boundary, not permission to tune a shadow by
eye. See
