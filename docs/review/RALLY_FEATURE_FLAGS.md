# Rally feature flags -- the reasoning behind each switch

Relocated verbatim from `scripts/simulation/rally_feature_flags.gd`, which was
725 comment lines around 31 constants -- 96% prose, the highest ratio in the
repository. Nothing here is new writing and nothing was summarised away; the
file now carries a NOTE and a pointer per flag, and this is what those point at.

The order is the file's own. A flag with no section below never had prose.

## `ENABLE_CONTINUOUS_RECEPTION_EVENTS`
...
## `ENABLE_CONTINUOUS_BLOCK_EVENTS`
...
## `ENABLE_DELIVERED_SET_SHOT_CHOICE`
...
## `ENABLE_OPPONENT_APPROACH_WINDOW`
